defmodule XMLStreamTools.ToMapTest do
  ...
  test "test 4" do
    xml = "<ns:foo a='1'>text0<bar a='1'>message</bar>text 1<bar b='2'>other message</bar> text 2</ns:foo>"
    result = parse_to_map(xml)
    assert result == [
      %{
        :_meta => %{tag: "foo", loc: {{1, 0}, 1}, order: [:_text, "bar", :_text, "bar", :_text] },
        :_namespace => "ns", 
        :a => "1",
  ...
  test "test 5" do
    xml = "<ns0:foo a='1'>text0<ns1:bar a='1'>message</ns1:bar>text 1<ns2:bar b='2'>other message</ns2:bar> text 2</ns:foo>"
