defmodule XMLStreamTools.ToMap do
      ...
      private: (
        ...
        |> Map.merge(
          %{
            ...
            attr_reduce: Keyword.get(private, :attr_reduce, fn attr_list ->
  ...
  def update_order([], _), do: []   # no parent, so nothing to do
...
defmodule XMLStreamTools.ToMap.FormatterDefault do
  ...
  def meta(acc, %{loc: loc, order: order, tag: tag}, %{meta_enabled: true}), do: [{:_meta, %{loc: loc, order: order, tag: tag}} | acc]
  ...
  def text(acc, %{text: text}, %{text_enabled: true, text_reduce: fun}), do: [ {:_text, Enum.reverse(text) |> fun.()} | acc]
  ...
  def tag_reduce(order, obj, acc) do
    order
    |> Enum.filter(fn v when is_binary(v) -> true; _ -> false end)
  ...
  def insert_obj(obj, tag, parent, [_|_] = parent_obj), do: Map.put(parent, tag, [ obj | parent_obj])
